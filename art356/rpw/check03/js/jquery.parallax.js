...
(function(jQuery, undefined) {
	...
	function layerNegative(xp ,yp) {
		return [xp?xp<0?-1:1:0,yp?yp<0?-1:1:0]
	}
	...
	function layerCss(parallax, px, negative, offset, size, position, pointer) {
		var pos = [],
		    cssPosition,
		    cssMargin,
		    x = 2,
		...
		while (x--) {
			if (parallax[x]) {
				pos[x] = parallax[x] * negative[x] * pointer[x] + offset[x];
	...
	jQuery.fn.parallax = function(o){
		...
		return this.each(function(i) {
			var node      = this,
				...
				negative  = layerNegative( opts.xparallax, opts.yparallax ),
			...
			function updateCss(newPointer) {
				var css = layerCss(parallax, px, negative, offset, size, position, newPointer);
				elem.css(css);
				pointer = newPointer;
			}
